...
%
For benchmark evaluation, we use the LM Evaluation Harness \cite{eval-harness} framework as the primary tool for running standardized model assessments. 
...
before and after each xLSTM block using hooks, and computing the corresponding values using the mathematical 
...
%
In the experimental setup involving memory caching, we build upon an existing xLSTM caching implementation provided in the model repository, 
adapting it to support our use case. This allows us to reuse previously computed memory states during question answering,
reducing redundant computation during inference. We restrict decoding to a maximum of 25 new tokens and use a temperature of 1.0 
