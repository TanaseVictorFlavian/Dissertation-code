...
with recurrent connections. Unlike traditional feedforward neural networks, which process each input without a 
concept of order,  
RNNs re-use a hidden state that gets updated after each time step, allowing information from past inputs to influence 
future predictions. \\ \\
%
The key idea behind recurrent architectures is the reuse of the same parameters at every time step. 
Given an input sequence, the recurrent state is updated recursively using the current input to update 
the previous hidden state, capturing temporal dependencies within the data.\\ \\ 
%
\begin{figure}[H]
    \centering
...
\end{figure}
\noindent The operation of a standard RNN can be formally described through its hidden state update in equation~(\ref{eq:vanilla-rnn-equation}).
...
\\ \\ 
To handle the exploding gradient problem, Pascanu et al.~\cite{pascanu2013difficulty} introduced 
\textit{gradient clipping}, a procedure used during training that doesn't allow gradients to go past a certain threshold
...
which led to the development of Long Short-Term Memory 
...
The \textbf{Long Short-Term Memory (LSTM)}(Hochreiter and Schmidhuber \cite{10.5555/2998981.2999048}) architecture, 
introduced in the 1990s,
was developed to improve on the limitations of standard RNNs which failed to maintain a long-term memory. 
The key innovation of the architecture is the 
introduction of a dedicated memory state that can preserve information across many time steps. 
Instead of relying solely on the hidden state, 
the LSTM uses a set of gating structures that control how information is retrieved, stored, and written to memory. 
%
\begin{figure}[H]
    \centering
...
previous hidden state, is incorporated into the cell state. The output gate (Eq:~\ref{eq:lstm_output_gate}) what information from the
updated cell state is transmitted as the next hidden state.
...
\noindent Overall, the introduction of a dedicated memory state and gating 
improve the standard RNN by being able to capture longer contexts. 
...
this efficiency, the mLSTM entirely abandons hidden-hidden recurrent connections allowing parallelization.
%
\begin{align}
