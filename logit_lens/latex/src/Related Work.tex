...
\subsection{Addressing Classic LSTM Limitations}
...
the sLSTM doesn't improve the sequential processing 
bottleneck of the classic LSTM. 

% \subsection{mLSTM}
% The mLSTM (matrix LSTM) fundamentally shifts the LSTM paradigm to solve both the storage capacity and parallelization bottlenecks [5]. To overcome the limited capacity of scalar states, the mLSTM expands the memory cell into a matrix $C \in \mathbb{R}^{d \times d}$ [11]. Crucially, to achieve this efficiency, the mLSTM entirely abandons memory mixing (the hidden-hidden recurrent connections), rendering the architecture fully parallelizable [5]. 

% Drawing heavily on Bidirectional Associative Memory (BAM) principles and Transformer terminology, the mLSTM stores information as key-value pairs ($k_t$, $v_t$) using a covariance (outer product) update rule, and retrieves it using a query vector ($q_t$) [11]. 

% \subsubsection{Forward Pass and Update Rules}
% At any given time step $t$, the inputs are linearly projected into queries, keys, and values [12]:
% \begin{align}
%     q_t &= W_q x_t + b_q \\
...
% The mLSTM retains the core gating mechanisms but uses exponential activations for the input gate and either a logistic sigmoid or exponential activation for the forget gate [12]:
% \begin{align}
%     i_t &= \exp(w_i^\top x_t + b_i) \\
%     f_t &= \sigma(w_f^\top x_t + b_f) \quad \text{OR} \quad \exp(w_f^\top x_t + b_f) \\
%     o_t &= \sigma(W_o x_t + b_o)
% \end{align}

% Instead of a scalar addition, the memory cell state $C_t$ is updated via a covariance update rule, where the forget gate acts as a decay rate and the input gate acts as a learning rate scaling the outer product of the value and key vectors [8, 12, 13]:
% \begin{equation}
%     C_t = f_t C_{t-1} + i_t v_t k_t^\top
% \end{equation}

% Because exponential gates can yield large values, the mLSTM maintains a normalizer state $n_t$ that tracks the weighted sum of key vectors, effectively keeping a record of the strength of the gates [8, 12]:
% \begin{equation}
%     n_t = f_t n_{t-1} + i_t k_t
% \end{equation}

% Finally, information retrieval is performed by multiplying the memory matrix $C_t$ with the query $q_t$ [11, 12]. To ensure stability, this result is normalized by the absolute projection of the query onto the normalizer state (bounded by 1 to prevent division by zero), and then gated by the output gate [8, 12]:
% \begin{align}
%     \tilde{h}_t &= \frac{C_t q_t}{\max\{|n_t^\top q_t|, 1\}} \\
...
% % -the of the lstm(equations) as the response to the exploding/vanishing gradient \\ X
