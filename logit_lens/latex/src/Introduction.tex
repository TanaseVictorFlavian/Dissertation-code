...
natural language processing tasks. This paradigm scaled rapidly, progressing from hundreds
of millions of parameters to trillions for enterprise-scale models. In parallel, 
...
%
Despite their success, transformers suffer from a fundamental computational bottleneck:
self-attention scales quadratically ($\mathcal{O}(N^2)$) relative to the sequence length, making long-context
...
problem of the transformers and the storage bottlenecks of traditional LSTMs, and try to achieve comparable performance while 
...
modelling. While analyzing the current state of research on recurrent neural networks, the xLSTM architecture, which 
...
to self-attention. \\ \\ 
%
While the performance of transformer language models has been extensively studied, a substantial body of 
...
of the xLSTM cells that contribute to the hidden state values.\\ \\ 
%
We establish an empirical baseline by evaluating the pretrained xLSTM-7B model across standard 
datasets, specifically PIQA\cite{bisk2020piqa}, Winogrande\cite{sakaguchi2021winogrande}, ARC-Challenge\cite{clark2018arc}, ARC-Easy\cite{clark2018arc}, 
...
and represents an assessment of its capabilities before 
